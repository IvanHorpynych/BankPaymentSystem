...
insert into USER (ROLE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, PASSWORD) values
  (1, 'John', 'Ukraine', 'ivan.horpynych@gmail.com', '+380661715108',
...
insert into ACCOUNT (USER_ID, BALANCE, TYPE_ID, STATUS_ID) values ((select ID
                                                                    ...
                                                                    where (select id
                                                                           from role
                                                                           where NAME = 'USER') = ROLE_ID), 2200,
                                                                   (select ID
                                                                    from ACCOUNT_TYPE
                                                                    where NAME = 'DEPOSIT'), (select ID
                                                                                              from STATUS
                                                                                              where NAME = 'ACTIVE'));
insert into DEPOSIT_ACCOUNT_DETAILS (ID, LAST_OPERATION, MIN_BALANCE, ANNUAL_RATE)
values (last_insert_id(), now(), 1000, 8.2);
...
insert into CARD (ACCOUNT_ID, PIN, CVV, EXPIRE_DATE, TYPE, STATUS_ID)
values (last_insert_id(), 1234, 666, '2019-1-01', 'MASTERCARD',(select ID from STATUS where NAME = 'ACTIVE'));
...
insert into CREDIT_REQUEST (USER_ID, INTEREST_RATE, VALIDITY_DATE, CREDIT_LIMIT, STATUS_ID) values
  ((select ID
    ...
    where (select id
           from role
           where NAME = 'USER') = ROLE_ID), 5.2, '2019-1-01', 5000, (select ID
                                                                     from STATUS
                                                                     where NAME = 'PENDING'));
...
insert into PAYMENT (AMOUNT, ACCOUNT_FROM, ACCOUNT_TO, OPERATION_DATE) VALUES (200, 2, 3, curdate());