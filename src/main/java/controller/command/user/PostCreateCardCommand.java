...
public class PostCreateCardCommand implements ICommand {
  ...
  private void validateRequestData(HttpServletRequest request, List<String> errors) {
    Util.validateField(new AccountNumberValidator(), request.getParameter(Attributes.DEBIT_ACCOUNT),
        errors);
  }

  private void validateAccount(HttpServletRequest request, List<String> errors) {
    Optional<Account> debitAccount = debitAccountService
        .findAccountByNumber(Long.valueOf(request.getParameter(Attributes.DEBIT_ACCOUNT)));
  ...
  private Card createCard(HttpServletRequest request) {
    Account debitAccount = debitAccountService
        .findAccountByNumber(Long.valueOf(request.getParameter(Attributes.DEBIT_ACCOUNT))).get();
    ...
    return Card.newBuilder().addAccount(debitAccount).addPin(CardPassGenerator.getRandomPin())
        .addCvv(CardPassGenerator.getRandomCvv()).addExpireDate(expireDate).addType(cardType)
        .addDefaultStatus().build();
  }
