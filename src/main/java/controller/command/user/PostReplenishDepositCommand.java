...
public class PostReplenishDepositCommand implements ICommand {
  ...
  private void validateAccountsBalance(HttpServletRequest request, List<String> errors) {
    ...
    Optional<DepositAccount> refillableAccountOptional = depositAccountService
  ...
  private Payment createPayment(HttpServletRequest request) {
    ...
    DepositAccount refillableAccount = depositAccountService
        .findAccountByNumber(Long.valueOf(request.getParameter(Attributes.REFILLABLE_ACCOUNT)))
        .get();
  ...
  private void checkChangingAccountData(Payment payment) {
     ...
     * AccountType.TypeIdentifier.DEPOSIT_TYPE.getId()) { ((DepositAccount)
     * payment.getAccountTo()).setMinBalance( ((DepositAccount)
     * payment.getAccountTo()).getMinBalance(). add(payment.getAmount()) ); }
     */
  }
  ...
  private void addDataToRequest(HttpServletRequest request) {
    ...
    refillableAccounts
        .add(depositAccountService.findAccountByNumber(refillableAccountNumber).get());
