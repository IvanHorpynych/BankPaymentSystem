...
public class PostNewCreditRequestCommand implements ICommand {
  ...
  public String execute(HttpServletRequest request, HttpServletResponse response)
    ...
    request.setAttribute(Attributes.INTEREST_RATE, request.getParameter(Attributes.INTEREST_RATE));

    request.setAttribute(Attributes.VALIDITY_DATE, new Date());

    request.setAttribute(Attributes.CREDIT_LIMIT, request.getParameter(Attributes.CREDIT_LIMIT));
  ...
  private void validateDataFromRequest(HttpServletRequest request, List<String> errors) {
    ...
    Util.validateField(new CreditLimitValidator(), request.getParameter(Attributes.CREDIT_LIMIT),
        errors);

    Util.validateField(new RateValidator(), request.getParameter(Attributes.INTEREST_RATE), errors);

    Util.validateField(new DateValidator(), request.getParameter(Attributes.VALIDITY_DATE), errors);

  }

  private void validateNumbers(HttpServletRequest request, List<String> errors) {
  ...
  private void validateDate(HttpServletRequest request, List<String> errors) {
  ...
  private CreditRequest createCreditRequest(HttpServletRequest request) {
    ...
    return CreditRequest.newBuilder().addAccountHolder(user).addInterestRate(interestRate)
        .addDefaultStatus().addValidityDate(validityDate).addCreditLimit(creditLimit).build();
  }
