...
public class PostNewPaymentCommand implements ICommand {
  ...
  public String execute(HttpServletRequest request, HttpServletResponse response)
    ...
    request.setAttribute(Attributes.CARD_TO, request.getParameter(Attributes.CARD_TO));
  ...
  private void validateDataFromRequest(HttpServletRequest request, List<String> errors) {

    Util.validateField(new CardNumberValidator(), getCleanCardNumber(request, Attributes.CARD_FROM),
        errors);

    Util.validateField(new CardNumberValidator(), request.getParameter(Attributes.CARD_TO), errors);

    Util.validateField(new CvvValidator(), request.getParameter(Attributes.CVV), errors);

    Util.validateField(new PinValidator(), request.getParameter(Attributes.PIN), errors);

    Util.validateField(new AmountValidator(), request.getParameter(Attributes.AMOUNT), errors);
  }

  private void validateCardCredentials(HttpServletRequest request, Card card, List<String> errors) {
  ...
  private void validateCardsStatus(Card cardFrom, Card cardTo, List<String> errors) {
    if (!cardFrom.isActive() || !cardTo.isActive())
      errors.add(CARD_STATUS_NOT_ACTIVE);
  }

  private void validateAccounts(Card cardFrom, Card cardTo, List<String> errors) {
  ...
  private void validateBalance(HttpServletRequest request, Card cardFrom, Card cardTo,
      List<String> errors) {
    Account senderAccount = cardFrom.getAccount();
  ...
  private void validateCoincidenceAccounts(Card cardFrom, Card cardTo, List<String> errors) {
  ...
  private Payment createPayment(HttpServletRequest request) {
    ...
    Payment payment =
        Payment.newBuilder().addAccountFrom(accountFrom).addCardNumberFrom(cardFrom.getCardNumber())
            .addAccountTo(accountTo).addAmount(amount).addDate(new Date()).build();

    return payment;
  }


  private Optional<Card> getCardOptFromRequest(HttpServletRequest request, String attribute) {
    long cardNumber;
