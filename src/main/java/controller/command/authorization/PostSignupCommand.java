...
public class PostSignupCommand implements ICommand {
  ...
  private final static String ACCOUNT_CREATED = "Created profile for user with email - ";

  private final UserService userService = ServiceFactory.getUserService();
  /*
   * private final AccountsService accountService = ServiceFactory.getAccountService(); private
   * final CardService cardService = ServiceFactory.getCardService();
   */
  ...
  public String execute(HttpServletRequest request, HttpServletResponse response)
    ...
    if (Util.isAlreadyLoggedIn(request.getSession())) {
      Util.redirectTo(request, response, bundle.getString("home.path"));
      return REDIRECTED;
    }
  ...
  private User getDataFromRequest(HttpServletRequest request) {
    return User.newBuilder().addEmail(request.getParameter(EMAIL_PARAM))
        ...
        .addPhoneNumber(request.getParameter(PHONE_PARAM)).build();
  }
  ...
  /*
   * private Account createNewAccount(User user) { Account tempAccount = new
   * Account(Account.DEFAULT_NUMBER, user, Account.DEFAULT_BALANCE, Account.DEFAULT_STATUS);
   * 
   * Account userAccount = accountService.createAccount(tempAccount);
   * 
   * return userAccount; }
   * 
   * private Card createNewCard(Account account, User user) { Card tempCard = Card.newBuilder()
   * .setCardNumber(Card.DEFAULT_NUMBER) .setAccount(account) .setCardHolder(user)
   * .setCvv(CardPassGenerator.getRandomCvv()) .setPin(CardPassGenerator.getRandomPin())
   * .setExpireDate(new Date()) .setType(Card.CardType.VISA) .build();
   * 
