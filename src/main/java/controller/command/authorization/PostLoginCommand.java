...
public class PostLoginCommand implements ICommand {
  ...
  public String execute(HttpServletRequest request, HttpServletResponse response)
    ...
    errors.addAll(validateUniquenessActiveUser(request.getSession(), userDto));
  ...
  private User getDataFromRequest(HttpServletRequest request) {
    return User.newBuilder().addEmail(request.getParameter(EMAIL_PARAM))
        .addPassword(request.getParameter(PASSWORD_PARAM)).build();
  }
  ...
  private List<String> validateData(User user) {
    ...
    /*
     * Check if entered password matches with user password only in case, when email and password is
     * valid
     */
    if (errors.isEmpty() && !userService.isCredentialsValid(user.getEmail(), user.getPassword())) {
  ...
  public List<String> validateUniquenessActiveUser(HttpSession session, User user) {
    ...
    Map<String, User> activeUserList =
        (Map<String, User>) session.getServletContext().getAttribute(Attributes.USER_LIST);
  ...
  private void addUserToContext(HttpSession session, User user) throws IOException {
    @SuppressWarnings("unchecked")
    Map<String, User> activeUserList =
        (Map<String, User>) session.getServletContext().getAttribute(Attributes.USER_LIST);
