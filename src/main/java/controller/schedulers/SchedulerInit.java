...
public class SchedulerInit {
  ...
  private static void init() {
    ...
    Trigger creditTrigger = newTrigger().withIdentity("creditTrigger", "mainGroup")
        .withSchedule(dailyAtHourAndMinute(1, 00)).build();

    Trigger depositTrigger = newTrigger().withIdentity("depositTrigger", "mainGroup")
        .withSchedule(dailyAtHourAndMinute(3, 00)).build();

    JobDetail creditJob = newJob(CreditJob.class).withIdentity("creditJob", "mainGroup").build();

    JobDetail depositJob = newJob(DepositJob.class).withIdentity("depositJob", "mainGroup").build();
