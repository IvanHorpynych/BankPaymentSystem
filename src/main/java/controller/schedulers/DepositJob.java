...
public class DepositJob implements Job {
  ...
  public void execute(JobExecutionContext context) throws JobExecutionException {
    List<DepositAccount> depositsAccounts = depositAccountService.findAllNotClosed();
    ...
    for (DepositAccount depositAccount : depositsAccounts) {
      ...
      if (depositAccount.getMinBalance().compareTo(BigDecimal.ZERO) > 0) {
        diffInMillis =
            Math.abs(currDate.getTime() - depositAccount.getLastOperationDate().getTime());
        difference = TimeUnit.DAYS.convert(diffInMillis, TimeUnit.MILLISECONDS);
        if (difference >= 29.5) {
          interestCharges =
              percentage(depositAccount.getMinBalance(), depositAccount.getAnnualRate() / 12);
