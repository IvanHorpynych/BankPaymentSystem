...
public class HibernateCardDao implements CardDao {
  ...
  public HibernateCardDao(Session session) {
   ...
   * .addAccountType(new AccountType(16, "DEBIT")).addBalance(BigDecimal.ONE) .addStatus(new
   * Status(1, "ACTIVE")).build();
   * 
   * System.out.println("Find all:"); HibernateCardDao mySqlCardDao = new
   * HibernateCardDao(HibernateUtil.getInstance()); ((HibernateCardDao)
   * mySqlCardDao).printCard(mySqlCardDao.findAll());
   * 
   * int random = (int) (Math.random() * 100);
   * 
   * System.out.println("Find one:"); System.out.println(mySqlCardDao.findOne(1000000000000000L));
   * 
   * System.out.println("find dy user:"); System.out.println(mySqlCardDao.findByUser(user));
   * 
   * System.out.println("Find by account"); for (Card card :
   * mySqlCardDao.findByAccount(debitAccount)) { System.out.println(card); }
   * 
   * System.out.println("Insert:"); Card card =
   * mySqlCardDao.insert(Card.newBuilder().addAccount(debitAccount).addPin(1111)
   * .addCvv(444).addExpireDate(new Date()).addType(Card.CardType.MASTERCARD) .addStatus(new
   * Status(1, "ACTIVE")).build()); List<Card> temp = new ArrayList<>(); card =
   * mySqlCardDao.findOne(card.getCardNumber()).get(); temp.add(card);
   * System.out.println("Find one:"); ((HibernateCardDao) mySqlCardDao).printCard(temp);
   * 
   * System.out.println("Find by account"); for (Card temp1 :
   * mySqlCardDao.findByAccount(debitAccount)) { System.out.println(temp1); }
   * 
   * System.out.println("update:"); card.setCvv(333); card.setType(Card.CardType.VISA);
   * mySqlCardDao.update(card);
   * 
   * temp.clear(); card = mySqlCardDao.findOne(card.getCardNumber()).get(); temp.add(card);
   * System.out.println("Find one:"); ((HibernateCardDao) mySqlCardDao).printCard(temp);
   * 
   * System.out.println("Find by user and status:"); ((HibernateCardDao) mySqlCardDao)
   * .printCard(mySqlCardDao.findByUserAndStatus(user, new Status(1, "ACTIVE")));
   * 
   * System.out.println("Delete:"); mySqlCardDao.delete(card.getCardNumber()); ((HibernateCardDao)
   * mySqlCardDao).printCard(mySqlCardDao.findAll());
   * 
   * }
   * 
   * protected void printCard(List<Card> list) { for (Card card : list) {
   * System.out.println("Card: " + card); System.out.println(); } }
   */
}
