...
public class MySqlPaymentDao implements PaymentDao {
  private final static String SELECT_ALL = "SELECT * FROM payment_details ";

  private final static String WHERE_ID = "WHERE id = ? ";

  private final static String WHERE_ACCOUNT = "WHERE (account_from  = ?) OR " + "(account_to = ?)";

  private final static String WHERE_USER = "WHERE (acc1_user_id = ?) OR " + "(acc2_user_id = ?)";

  private final static String WHERE_CARD_NUMBER = "WHERE card_number_from = ?";

  private final static String INSERT =
      "INSERT INTO payment (" + "amount, account_from, card_number_from, "
          + "account_to, operation_date) " + "VALUES (?, ?, ?, ?, ?) ";

  private final static String UPDATE = "UPDATE payment SET " + "amount = ?, operation_date = ? ";

  private final static String DELETE = "DELETE FROM payment ";
  ...
  public MySqlPaymentDao(Connection connection, DtoConverter<Payment> converter) {
  ...
  public Payment insert(Payment obj) {
    ...
    int id = (int) defaultDao.executeInsertWithGeneratedPrimaryKey(INSERT, obj.getAmount(),
        obj.getAccountFrom().getAccountNumber(),
        (obj.getCardNumberFrom() == 0L ? null : obj.getCardNumberFrom()),
        obj.getAccountTo().getAccountNumber(), TimeConverter.toTimestamp(obj.getDate()));
  ...
  public void update(Payment obj) {
    ...
    defaultDao.executeUpdate(UPDATE + WHERE_ID, obj.getAmount(),
        TimeConverter.toTimestamp(obj.getDate()), obj.getId());
  }
  ...
  public List<Payment> findByAccount(Long accountNumber) {
    ...
    return defaultDao.findAll(SELECT_ALL + WHERE_ACCOUNT, accountNumber, accountNumber);
  }
  ...
  public List<Payment> findByUser(User user) {
    ...
    return defaultDao.findAll(SELECT_ALL + WHERE_USER, user.getId(), user.getId());
  }

  @Override
  public List<Payment> findByCardNumber(long cardNumber) {
    return defaultDao.findAll(SELECT_ALL + WHERE_CARD_NUMBER, cardNumber);
  }
  ...
  public static void main(String[] args) {
    ...
    CreditAccount creditAccount1 =
        CreditAccount.newCreditBuilder().addAccountNumber(3).addAccountHolder(user)
