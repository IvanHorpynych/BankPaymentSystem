...
public class MySqlCardDao implements CardDao {
  private final static String SELECT_ALL = "SELECT * FROM card_details ";

  private final static String WHERE_CARD_NUMBER = "WHERE card_number = ? ";

  private final static String WHERE_ACCOUNT = "WHERE id = ? ";

  private final static String WHERE_USER = "WHERE user_id = ? ";

  private final static String AND_STATUS = " and card_status_id = ?";

  private final static String AND_TYPE = " and type_id = ?";

  private final static String INSERT = "INSERT INTO card"
      + "(account_id, pin, cvv, expire_date, type, status_id)" + "VALUES (?, ?, ?, ?, ?, ?) ";

  private final static String UPDATE =
      "UPDATE card SET " + "pin = ?, cvv = ?, expire_date = ?, type = ? ";

  private final static String UPDATE_STATUS = "UPDATE card SET " + "status_id = ? ";

  private final static String DELETE = "DELETE FROM card ";
  ...
  public MySqlCardDao(Connection connection, DtoConverter<Card> converter) {
  ...
  public Optional<Card> findOne(Long cardNumber) {
    return defaultDao.findOne(SELECT_ALL + WHERE_CARD_NUMBER, cardNumber);
  }
  ...
  public Card insert(Card obj) {
    ...
    obj.setCardNumber(
        defaultDao.executeInsertWithGeneratedPrimaryKey(INSERT, obj.getAccount().getAccountNumber(),
            obj.getPin(), obj.getCvv(), TimeConverter.toTimestamp(obj.getExpireDate()),
            obj.getType().toString(), obj.getStatus().getId()));
  ...
  public void update(Card obj) {
    ...
    defaultDao.executeUpdate(UPDATE + WHERE_CARD_NUMBER, obj.getPin(), obj.getCvv(),
        TimeConverter.toTimestamp(obj.getExpireDate()), obj.getType().toString(),
        obj.getCardNumber());
  }

  @Override
  public void delete(Long cardNumber) {
    defaultDao.executeUpdate(DELETE + WHERE_CARD_NUMBER, cardNumber);
  }
  ...
  public List<Card> findByAccount(Account account) {
    return defaultDao.findAll(SELECT_ALL + WHERE_ACCOUNT, account.getAccountNumber());
  }


  @Override
  public List<Card> findByUserAndStatus(User user, Status status) {
    return defaultDao.findAll(SELECT_ALL + WHERE_USER + AND_STATUS, user.getId(), status.getId());
  }
  ...
  public void updateCardStatus(Card card, int statusId) {
    ...
    defaultDao.executeUpdate(UPDATE_STATUS + WHERE_CARD_NUMBER, statusId, card.getCardNumber());
  }
  ...
  public static void main(String[] args) {
    ...
    Account debitAccount = Account.newBuilder().addAccountNumber(3).addAccountHolder(user)
    ...
    try {
      ...
      Card card = ((MySqlCardDao) mySqlCardDao).insert(Card.newBuilder().addAccount(debitAccount)
          .addPin(1111).addCvv(444).addExpireDate(new Date()).addType(Card.CardType.MASTERCARD)
          .addStatus(new Status(1, "ACTIVE")).build());
