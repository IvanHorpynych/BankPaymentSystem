...
public class MySqlCreditRequestDao implements CreditRequestDao {
  private final static String SELECT_ALL = "SELECT * FROM credit_request_details ";

  private final static String WHERE_REQUEST_NUMBER = "WHERE id = ? ";

  private final static String WHERE_USER = "WHERE user_id = ? ";

  private final static String WHERE_STATUS = "WHERE status_id = ? ";

  private final static String INSERT =
      "INSERT INTO credit_request " + "(user_id, interest_rate, status_id, "
          + "validity_date, credit_limit) " + "VALUES(?, ?, ?, ?, ?) ";

  private final static String UPDATE = "UPDATE credit_request SET "
      + "interest_rate = ?, status_id = ?, " + "validity_date = ?, credit_limit = ? ";

  private final static String UPDATE_STATUS = "UPDATE credit_request SET " + "status_id = ? ";

  private final static String DELETE = "DELETE FROM credit_request ";
  ...
  public MySqlCreditRequestDao(Connection connection, DtoConverter<CreditRequest> converter) {
  ...
  public CreditRequest insert(CreditRequest obj) {
    ...
    long requestNumber = defaultDao.executeInsertWithGeneratedPrimaryKey(INSERT,
        obj.getAccountHolder().getId(), obj.getInterestRate(), obj.getStatus().getId(),
        obj.getValidityDate(), obj.getCreditLimit());
  ...
  public void update(CreditRequest obj) {
    ...
    defaultDao.executeUpdate(UPDATE + WHERE_REQUEST_NUMBER, obj.getInterestRate(),
        obj.getStatus().getId(), obj.getValidityDate(), obj.getCreditLimit(),
        obj.getRequestNumber());
  }

  @Override
  public void delete(Long requestNumber) {
    defaultDao.executeUpdate(DELETE + WHERE_REQUEST_NUMBER, requestNumber);
  }
  ...
  public List<CreditRequest> findByStatus(long statusId) {
    ...
    return defaultDao.findAll(SELECT_ALL + WHERE_STATUS, statusId);
  }
  ...
  public void updateRequestStatus(CreditRequest request, int statusId) {
    ...
    defaultDao.executeUpdate(UPDATE_STATUS + WHERE_REQUEST_NUMBER, statusId,
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      CreditRequest creditRequest = mySqlCreditRequestDao.insert(CreditRequest.newBuilder()
