...
public class MySqlAccountTypeDao implements AccountTypeDao {
  private final static String SELECT_ALL =
      "SELECT id AS type_id, name AS type_name " + "FROM account_type ";

  private final static String INSERT = "INSERT INTO account_type (name) " + "VALUES(?);";

  private final static String UPDATE = "UPDATE account_type SET name = ? ";

  private final static String DELETE = "DELETE FROM account_type ";
  ...
  public MySqlAccountTypeDao(Connection connection, DtoConverter<AccountType> converter) {
  ...
  public AccountType insert(AccountType type) {
    ...
    int id = (int) defaultDao.executeInsertWithGeneratedPrimaryKey(INSERT, type.getName());
  ...
  public void update(AccountType type) {
    ...
    defaultDao.executeUpdate(UPDATE + WHERE_ID, type.getName(), type.getId());
  }
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      ((MySqlAccountTypeDao) mySqlAccountTypeDao).printAll(mySqlAccountTypeDao.findAll());
      System.out.println();
      ...
      AccountType accountType = mySqlAccountTypeDao.insert(new AccountType(0, "TEST"));
      ((MySqlAccountTypeDao) mySqlAccountTypeDao).printAll(mySqlAccountTypeDao.findAll());
