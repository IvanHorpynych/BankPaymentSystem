...
public class MySqlCreditAccountDao implements CreditAccountDao {
  private final static String SELECT_ALL = "SELECT * FROM credit_details ";

  private final static String WHERE_ACCOUNT_NUMBER = "WHERE id = ? ";

  private final static String WHERE_USER = "WHERE user_id = ? ";

  private final static String WHERE_NOT_CLOSED =
      "WHERE status_id != " + "(SELECT id FROM status where name = 'CLOSED') ";

  private final static String INSERT =
      "INSERT INTO account " + "(user_id, type_id, status_id, balance) " + "VALUES(?, ?, ?, ?) ";

  private final static String INSERT_DETAILS =
      "INSERT INTO credit_account_details " + "(id, credit_limit, interest_rate, "
          + "accrued_interest, " + "validity_date) " + "VALUES(?, ?, ?, ?, ?) ";

  private final static String UPDATE =
      "UPDATE credit_account_details SET " + "accrued_interest = ?, validity_date = ? ";

  private final static String UPDATE_STATUS = "UPDATE account SET " + "status_id = ? ";

  private final static String INCREASE_BALANCE = "UPDATE account SET " + "balance = balance + ? ";

  private final static String DECREASE_BALANCE = "UPDATE account SET " + "balance = balance - ? ";

  private final static String INCREASE_ACCRUED_INTEREST =
      "UPDATE credit_account_details SET " + "accrued_interest = accrued_interest + ? ";

  private final static String DECREASE_ACCRUED_INTEREST =
      "UPDATE credit_account_details SET " + "accrued_interest = accrued_interest - ? ";

  private final static String DELETE = "DELETE details, account FROM "
      + "credit_account_details AS details " + "JOIN account  USING(id) ";
  ...
  public MySqlCreditAccountDao(Connection connection, DtoConverter<CreditAccount> converter) {
  ...
  public Optional<CreditAccount> findOne(Long accountNumber) {
    return defaultDao.findOne(SELECT_ALL + WHERE_ACCOUNT_NUMBER, accountNumber);
  }
  ...
  public CreditAccount insert(CreditAccount account) {
    ...
    long accountNumber =
        defaultDao.executeInsertWithGeneratedPrimaryKey(INSERT, account.getAccountHolder().getId(),
            account.getAccountType().getId(), account.getStatus().getId(), account.getBalance());

    account.setAccountNumber(accountNumber);

    defaultDao.executeUpdate(INSERT_DETAILS, account.getAccountNumber(), account.getCreditLimit(),
        account.getInterestRate(), account.getAccruedInterest(),
  ...
  public void update(CreditAccount account) {
     ...
     * Objects.requireNonNull(account);
     * 
     * defaultDao.executeUpdate( UPDATE + WHERE_ACCOUNT_NUMBER, account.getAccruedInterest(),
     * TimeConverter.toTimestamp(account.getValidityDate()), account.getAccountNumber() );
     */
  }

  @Override
  public void delete(Long accountNumber) {
    defaultDao.executeUpdate(DELETE + WHERE_ACCOUNT_NUMBER, accountNumber);
  ...
  public List<CreditAccount> findByUser(User user) {
    ...
    return defaultDao.findAll(SELECT_ALL + WHERE_USER, user.getId());
  }

  @Override
  public List<CreditAccount> findAllNotClosed() {
    return defaultDao.findAll(SELECT_ALL + WHERE_NOT_CLOSED);
  }
  ...
  public void increaseBalance(CreditAccount account, BigDecimal amount) {
    ...
    defaultDao.executeUpdate(INCREASE_BALANCE + WHERE_ACCOUNT_NUMBER, amount,
  ...
  public void decreaseBalance(CreditAccount account, BigDecimal amount) {
    ...
    defaultDao.executeUpdate(DECREASE_BALANCE + WHERE_ACCOUNT_NUMBER, amount,
        account.getAccountNumber());
  }
  ...
  public void updateAccountStatus(CreditAccount account, int statusId) {
    ...
    defaultDao.executeUpdate(UPDATE_STATUS + WHERE_ACCOUNT_NUMBER, statusId,
        account.getAccountNumber());
  }
  ...
  public void increaseAccruedInterest(CreditAccount account, BigDecimal amount) {
    ...
    defaultDao.executeUpdate(INCREASE_ACCRUED_INTEREST + WHERE_ACCOUNT_NUMBER, amount,
        account.getAccountNumber());
  }
  ...
  public void decreaseAccruedInterest(CreditAccount account, BigDecimal amount) {
    ...
    defaultDao.executeUpdate(DECREASE_ACCRUED_INTEREST + WHERE_ACCOUNT_NUMBER, amount,
        account.getAccountNumber());
  }
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      CreditAccount creditAccount = (CreditAccount) mySqlCreditAccountDao.insert(CreditAccount
          .newCreditBuilder().addAccountHolder(user).addAccountType(new AccountType(4, "CREDIT"))
          .addBalance(BigDecimal.ONE).addCreditLimit(BigDecimal.TEN).addInterestRate(2L)
          .addAccruedInterest(BigDecimal.ZERO).addValidityDate(new Date())
