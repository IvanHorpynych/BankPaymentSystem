...
public class MySqlStatusDao implements StatusDao {
  private final static String SELECT_ALL =
      "SELECT id AS status_id, name AS status_name " + "FROM status ";

  private final static String INSERT = "INSERT INTO status (name) " + "VALUES(?);";

  private final static String UPDATE = "UPDATE status SET name = ? ";

  private final static String DELETE = "DELETE FROM status ";
  ...
  public MySqlStatusDao(Connection connection, DtoConverter<Status> converter) {
  ...
  public Status insert(Status status) {
    ...
    int id = (int) defaultDao.executeInsertWithGeneratedPrimaryKey(INSERT, status.getName());
  ...
  public void update(Status status) {
    ...
    defaultDao.executeUpdate(UPDATE + WHERE_ID, status.getName(), status.getId());
  }
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      Status accountType = mySqlStatusDao.insert(new Status(0, "TEST"));
      ((MySqlStatusDao) mySqlStatusDao).printAll(mySqlStatusDao.findAll());
