...
public class CreditAccountDtoConverter implements DtoConverter<CreditAccount> {
  ...
  public CreditAccount convertToObject(ResultSet resultSet, String tablePrefix)
    ...
    return CreditAccount.newCreditBuilder()
        ...
        .addCreditLimit(resultSet.getBigDecimal(tablePrefix + CREDIT_LIMIT_FIELD))
        .addInterestRate(resultSet.getFloat(tablePrefix + INTEREST_RATE_FIELD))
        .addAccruedInterest(resultSet.getBigDecimal(tablePrefix + ACCRUED_INTEREST_FIELD))
        .addValidityDate(
