...
public class CreditRequestDtoConverter implements DtoConverter<CreditRequest> {
  ...
  public CreditRequestDtoConverter(DtoConverter<User> userConverter,
  ...
  public CreditRequest convertToObject(ResultSet resultSet, String tablePrefix)
    ...
    CreditRequest creditRequest = CreditRequest.newBuilder()
        .addRequestNumber(resultSet.getLong(tablePrefix + REQUEST_NUMBER_FIELD))
        .addAccountHolder(accountHolder)
        .addInterestRate(resultSet.getFloat(tablePrefix + INTEREST_RATE_FIELD)).addStatus(status)
        .addValidityDate(
            TimeConverter.toDate(resultSet.getTimestamp(tablePrefix + VALIDITY_DATE_FIELD)))
        .addCreditLimit(resultSet.getBigDecimal(tablePrefix + CREDIT_LIMIT_FIELD)).build();
