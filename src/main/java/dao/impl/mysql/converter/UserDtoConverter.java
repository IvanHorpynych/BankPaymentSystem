...
public class UserDtoConverter implements DtoConverter<User> {
  ...
  @Override
  public User convertToObject(ResultSet resultSet, String tablePrefix) throws SQLException {

    accountOrder = accountOrderIdentifier(tablePrefix);

    Role role = roleConverter.convertToObject(resultSet, accountOrder);

    User user = User.newBuilder().addId(resultSet.getLong(tablePrefix + ID_FIELD)).addRole(role)
        .addFirstName(resultSet.getString(tablePrefix + FIRST_NAME_FIELD))
        .addLastName(resultSet.getString(tablePrefix + LAST_NAME_FIELD))
        .addEmail(resultSet.getString(tablePrefix + EMAIL_FIELD))
        .addPassword(resultSet.getString(tablePrefix + PASSWORD_FIELD))
        .addPhoneNumber(resultSet.getString(tablePrefix + PHONE_NUMBER_FIELD)).build();
